...
description: >
  ...
  subfolders, identifies and removes temporary/duplicate files, and updates CLAUDE.md file
  inventories across the folder hierarchy. Triggered when the user asks to tidy up, organize,
  or clean a legal project folder.
allowed-tools:
  - Read
  - Write
  - Edit
...
Read `phase4-claudemd-logic.md` in this skill's directory for the complete update logic. It covers:

- Managed section marker format and templates
- File inventory and subproject index section formats with examples
- Three-way case logic for handling existing CLAUDE.md files (Case A: none exists, Case B: has managed sections, Case C: has unmanaged sections)
